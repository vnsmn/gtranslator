...
public class PhraseGoogleFormater extends DefaultGoogleFormater {
	@Override
	public String format(String jsonText, boolean isAddition) {
		...
		sb.append("\n----------\n");
		Set<String> dublicate = new TreeSet<>();
		t = (Tree) t.content.get(5);
		for (Object l : t.content) {
			if (l instanceof Tree) {
				Tree t1 = (Tree) l;
				String head = (String) t1.content.get(0);
				if (dublicate.contains(head)) {
					continue;
				}
				dublicate.add(head);
				sb.append("-----");
				...
				for (Object l1 : t1.content) {
					if (l1 instanceof Tree) {
						...
						if (t2.content.size() > 3) {
							sb.append(" - ");
							BigDecimal bd = (BigDecimal) t2.content
									.get(t2.content.size() - 1);
							MathContext mc = new MathContext(2, RoundingMode.UP);
							sb.append(bd.round(mc)
